...
public class EditContent extends Activity implements OnClickListener{
	...
	private ContentContext globalCc;	
...
//				 	process fetching content
					progressBarStatus = displayContent(context);//progressBarStatus = displayContent(context, globService);					
...
//					 	process fetching content
	...
	private int updateCc(String titleText, String descriptionText) {
		//update blog with contentn context object with loaded path and 'mobiBlogger' key
		ContentContext cc = new ContentContext(GeneralConstants.DATA_TABLE_NAME);
		cc.setPath(path);
		try {
			//set title, description and photo fields of the content context object 
	...
	private int deleteCc(ContentContext ccUpdate) {
		//delete cc object
		ContentContext cc = new ContentContext(GeneralConstants.DATA_TABLE_NAME);
		cc.setPath(path);
		try {
